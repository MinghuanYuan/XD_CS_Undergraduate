...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        H = np.eye(3)
        if m == eTranslate:
            pair = random.sample(matches, 1)
            x1, y1 = f1[pair[0].queryIdx].pt
            x2, y2 = f2[pair[0].trainIdx].pt
            H[0, 2] = x2 - x1
            H[1, 2] = y2 - y1
        elif m == eHomography:
            pairs = random.sample(matches, 4)
            H = computeHomography(f1, f2, pairs)
        InliersIndex = getInliers(f1, f2, matches, H, RANSACthresh)
        if len(MaxInliersIndex) < len(InliersIndex):
            MaxInliersIndex = InliersIndex
    M = leastSquaresFit(f1, f2, matches, m, MaxInliersIndex)
    # raise Exception("TODO in alignment.py not implemented")
    # TODO-BLOCK-END
    # END TODO
    return M
